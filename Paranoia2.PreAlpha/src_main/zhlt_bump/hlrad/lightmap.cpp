...
static void     CalcFaceExtents(lightinfo_t* l)
{
	...
	{
		if ((l->texsize[0] > MAX_SURFACE_EXTENT) || (l->texsize[1] > MAX_SURFACE_EXTENT) || l->texsize[0] < 0 || l->texsize[1] < 0 )
		{
			ThreadLock();
...
#ifdef HLRAD_HULLU
                        					VectorMultiply(add_diffuse,transparency,add_diffuse);
                        					VectorMultiply(add_ambient,transparency,add_ambient);
#endif

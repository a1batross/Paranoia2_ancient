...
#ifdef ZHLT_NEW_BSP_VERSION
int             g_numclipnodes[MAX_MAP_HULLS];
dclipnode_t     g_dclipnodes[MAX_MAP_HULLS][MAX_MAP_CLIPNODES];
int             g_dclipnodes_checksum[MAX_MAP_HULLS];
#else
int             g_numclipnodes;
...
static void     SwapBSPFile(const bool todisk)
{
    ...
    //
#ifdef ZHLT_NEW_BSP_VERSION
    for (c = 1; c < MAX_MAP_HULLS; c++)
    {
        for (i = 0; i < g_numclipnodes[c]; i++)
        {
            g_dclipnodes[c][i].planenum = LittleLong(g_dclipnodes[c][i].planenum);
            g_dclipnodes[c][i].children[0] = LittleShort(g_dclipnodes[c][i].children[0]);
            g_dclipnodes[c][i].children[1] = LittleShort(g_dclipnodes[c][i].children[1]);
        }
    }
...
#ifdef ZHLT_NEW_BSP_VERSION
    g_numclipnodes[1] = CopyLump(LUMP_CLIPNODES, g_dclipnodes[1], sizeof(dclipnode_t), header);
    g_numclipnodes[2] = CopyLump(LUMP_CLIPNODES2, g_dclipnodes[2], sizeof(dclipnode_t), header);
    g_numclipnodes[3] = CopyLump(LUMP_CLIPNODES3, g_dclipnodes[3], sizeof(dclipnode_t), header);
#else
...
#ifdef ZHLT_NEW_BSP_VERSION
    g_dclipnodes_checksum[1] = FastChecksum(g_dclipnodes[1], g_numclipnodes[1] * sizeof(dclipnode_t));
    g_dclipnodes_checksum[2] = FastChecksum(g_dclipnodes[2], g_numclipnodes[2] * sizeof(dclipnode_t));
    g_dclipnodes_checksum[3] = FastChecksum(g_dclipnodes[3], g_numclipnodes[3] * sizeof(dclipnode_t));
#else
...
#ifdef ZHLT_NEW_BSP_VERSION
    AddLump(LUMP_CLIPNODES,  g_dclipnodes[1],   g_numclipnodes[1] * sizeof(dclipnode_t), header, bspfile);
    AddLump(LUMP_CLIPNODES2, g_dclipnodes[2],   g_numclipnodes[2] * sizeof(dclipnode_t), header, bspfile);
    AddLump(LUMP_CLIPNODES3, g_dclipnodes[3],   g_numclipnodes[3] * sizeof(dclipnode_t), header, bspfile);
#else
...
#ifdef ZHLT_NEW_BSP_VERSION
    totalmemory += ArrayUsage("clipnodes1", g_numclipnodes[1], ENTRIES(g_dclipnodes[1]), ENTRYSIZE(g_dclipnodes[1]));
    totalmemory += ArrayUsage("clipnodes2", g_numclipnodes[2], ENTRIES(g_dclipnodes[2]), ENTRYSIZE(g_dclipnodes[2]));
    totalmemory += ArrayUsage("clipnodes3", g_numclipnodes[3], ENTRIES(g_dclipnodes[3]), ENTRYSIZE(g_dclipnodes[3]));
#else

...
extern BOOL	UTIL_TraceCustomDecal( TraceResult *pTrace, const char *name, int persistent = 0 ); // Wargon: Это должно быть объявлено здесь чтобы можно было рисовать кастомне декали из кода монстриков.
extern void	UTIL_RestoreCustomDecal( const Vector &vecPos, const Vector &vecNormal, int entityIndex, int modelIndex, const char *name, int flags );
extern BOOL	UTIL_StudioDecalTrace( const Vector &vecSrc, TraceResult *pTrace, const char *name, int flags = FDECAL_CLIPTEST );
extern void	UTIL_RestoreStudioDecal( const Vector &vecEnd, const Vector &vecSrc, int entityIndex, int modelIndex, const char *name, int flags, struct modelstate_s *state );

// Lev
int                 TraceTexturetype( Vector vecSrc, Vector vecEnd, CBaseEntity *pEntity );
extern void         UTIL_CreateDecal( TraceResult *pTrace, int decalindex );
extern void         UTIL_BulletImpact(TraceResult *pTrace,  Vector vecSrc, Vector vecEnd );
extern void         UTIL_KnifeImpact( TraceResult *pTrace, Vector vecSrc, Vector vecEnd );
//

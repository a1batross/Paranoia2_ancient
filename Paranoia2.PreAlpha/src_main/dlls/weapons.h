...
#define MP5_DEFAULT_AMMO		30  // buz: used in paranoia
...
#define ITEM_FLAG_ALLOW_FIREMODE	32
...
public:
    ...
    virtual void WeaponSetMode( int impulse );
	...
	// Wargon: ѕеременные дл€ юзабельности оружий.
	...
	int		m_iWeaponFireMode;
	int		m_iWorldModel;	// keep modelname for dropping
};
...
public:
	...
	// Wargon: ѕеременные дл€ юзабельности патронов.
	virtual int Save( CSave &save );
	virtual int Restore( CRestore &restore );
	static TYPEDESCRIPTION m_SaveData[];
	void EXPORT DefaultUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value ) { DefaultTouch( pActivator ); }
...
extern void DecalGunshot( TraceResult *pTrace, int iBulletType, const Vector &vecSrc );
...
//	void EXPORT SwingAgain( void );
	...
	int m_iSwing;
	TraceResult m_trHit;
	Vector m_vecSrc;	// for studiodecals
};
...
// Wargon: ќбъ€влени€ CRpg и CRpgRocket перемещены в rpg.cpp.
/*

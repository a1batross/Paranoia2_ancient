...
void CGroza::Reload1()
{
    ...
    {
	DefaultReload( GROZA_MAX_CLIP, GROZA_RELOAD2, 2.3 );
    }
...
{	
    ...
    if ( m_iClip <= 0 )
	    m_iGrozaMag = PRECACHE_MODEL ("models/w_grammo_empty.mdl");
    else
        m_iGrozaMag = PRECACHE_MODEL ("models/w_grammo.mdl");
	...
	EjectBrass ( pev->origin + gpGlobals->v_up * -4 + gpGlobals->v_forward * 1 + gpGlobals->v_right * 10,vecClipVelocity, pev->angles.y, m_iGrozaMag, TE_BOUNCE_NULL);
}*/

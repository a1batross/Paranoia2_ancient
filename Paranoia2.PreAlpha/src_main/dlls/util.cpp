...
// buz
BOOL UTIL_TraceCustomDecal( TraceResult *pTrace, const char *name, int persistent ) // Wargon: Значение по умолчанию прописано в util.h.
{
	short entityIndex;
	short modelIndex = 0;
	byte flags = 0;
...
void UTIL_RestoreCustomDecal( const Vector &vecPos, const Vector &vecNormal, int entityIndex, int modelIndex, const char *name, int flags )
{
...
BOOL UTIL_StudioDecalTrace( const Vector &vecSrc, TraceResult *pTrace, const char *name, int flags )
{
	short entityIndex;
...
void UTIL_RestoreStudioDecal( const Vector &vecEnd, const Vector &vecSrc, int entityIndex, int modelIndex, const char *name, int flags, modelstate_t *state )
{
...
void UTIL_BloodDecalTrace( TraceResult *pTrace, int bloodColor )
{
	if ( UTIL_ShouldShowBlood( bloodColor ) )
	{
		if ( bloodColor == BLOOD_COLOR_RED )
			UTIL_TraceCustomDecal( pTrace, "redblood" );
		else
			UTIL_TraceCustomDecal( pTrace, "yellowblood" );
	}
}
...
void UTIL_BloodStudioDecalTrace( const Vector &vecSrc, TraceResult *pTrace, int bloodColor )
{
	...
	{
		if ( bloodColor == BLOOD_COLOR_RED )
			UTIL_StudioDecalTrace( vecSrc, pTrace, "bodyhurt_red" );
		else
			UTIL_StudioDecalTrace( vecSrc, pTrace, "bodyhurt_yellow" );
	}
}
...
void UTIL_BulletImpact( TraceResult *pTrace, Vector vecSrc, Vector vecEnd )
{
	CBaseEntity *pEntity = CBaseEntity::Instance( pTrace->pHit );
	...
	case CHAR_TEX_COMPUTER:	
		UTIL_CreateDecal( pTrace, 0 );       
		break;
	case CHAR_TEX_METAL:
	case CHAR_TEX_GRATE:
	case CHAR_TEX_VENT:	
	UTIL_CreateDecal( pTrace, 1 );
		UTIL_Sparks( pTrace->vecEndPos );
		break;  
	case CHAR_TEX_WOOD:
	case CHAR_TEX_WOODS:	
		UTIL_CreateDecal( pTrace, 2 );
		break;
	case CHAR_TEX_GLASS: 
		UTIL_CreateDecal( pTrace, 3 );
		break;
	}
}

void UTIL_KnifeImpact( TraceResult *pTrace, Vector vecSrc, Vector vecEnd )
{
	CBaseEntity *pEntity = CBaseEntity::Instance( pTrace->pHit );
	...
	case CHAR_TEX_VENT:	
		...
		{
		case 1:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/metal1.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 2:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/metal2.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 3:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/metal3.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 4:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/metal4.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		}
		break;  
	case CHAR_TEX_WOOD:
	case CHAR_TEX_WOODS:
		...
		{
		case 1:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/wood1.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 2:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/wood2.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 3:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/wood3.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 4:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/wood4.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		}
		break;
	case CHAR_TEX_GLASS:
		...
		{
		case 1:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/debris/glass1.wav.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 2:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/debris/glass2.wav.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		case 3:
			UTIL_EmitAmbientSound( ENT(0), pTrace->vecEndPos, "debris/debris/glass3.wav.wav", 0.8, ATTN_NORM, 0, 96 + RANDOM_LONG(0,0xf));
			break;
		}	
		break;
	}
...
void UTIL_CreateDecal( TraceResult *pTrace, int decalindex )
{
	if( pTrace->flFraction == 1.0 )
		return;

	if( decalindex < 0 ) return;
	
	// Bullets
	if( decalindex == 0 ) // concrete shot
	{
		UTIL_TraceCustomDecal( pTrace, "shot" );
	}

	if( decalindex == 1 ) // metal shot
	{
		UTIL_TraceCustomDecal( pTrace, "shot_metal" );
	...
	if( decalindex == 2 ) // wood shot
	{
		UTIL_TraceCustomDecal( pTrace, "shot_wood" );
	...
	if( decalindex == 3 ) // glass shot
	{
		UTIL_TraceCustomDecal( pTrace, "shot_glass" );

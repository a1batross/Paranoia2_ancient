...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "ammo_tt33box" );
	...
	// buz: dont precache Half-life weapons
#if 0
	ALERT(at_console, "Precaching Half-Life weapons\n");
...
TYPEDESCRIPTION	CBasePlayerItem::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayerItem, m_iCurrCaps, FIELD_INTEGER ), // Wargon: Сохранение m_iCurrCaps.
	DEFINE_FIELD( CBasePlayerItem, m_iWorldModel, FIELD_MODELNAME ),
};
...
TYPEDESCRIPTION	CBasePlayerWeapon::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayerWeapon, m_iWeaponFireMode, FIELD_INTEGER ),
...
{
	m_iWorldModel = pev->model;	// keep worlmodel name for right dropping
...
void CBasePlayerItem::WeaponSetMode( int impulse )
{
	if( iFlags() & ITEM_FLAG_ALLOW_FIREMODE )
	{  
		m_iWeaponFireMode = !m_iWeaponFireMode;
		EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/weapon_chengemode.wav", 0.8, ATTN_NORM);
	}
}
...
void CBasePlayerItem::AttachToPlayer ( CBasePlayer *pPlayer )
{
	...
	pev->effects |= EF_NODRAW; // ??
	// buz: dont override gun's modelindex (hack)
...
//		skiplocal = 1;
//	else
		skiplocal = 0; // buz: predictig system is disabled
...
// buz
// precondition: item's pev->modelindex should be set to view model index
void CBasePlayerWeapon :: WeaponDelay(int iAnim)
{
	float delay = GetSequenceTime( GET_MODEL_PTR(ENT(pev)), iAnim );
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + delay;
	m_flNextSecondaryAttack = m_flNextPrimaryAttack = m_pPlayer->m_flNextAttack;
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + delay + 0.5;
}

void CBasePlayerWeapon :: WeaponDelayTime( float time )
{
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + time;
	m_flNextSecondaryAttack = m_flNextPrimaryAttack = m_pPlayer->m_flNextAttack;
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + time + 0.5;
}

extern int m_iEnableChangeFifeMode;
...
//	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 1.0;
	...
	// buz: make weapon world model same as view model (it's anyway has EF_NODRAW flag)
	SET_MODEL(ENT(pev), szViewModel);
	//pev->modelindex = (*g_engfuncs.pfnModelIndex)(szViewModel);
	//pev->model = MAKE_STRING( szViewModel );

	// buz: deploy time based on animation length
	WeaponDelay(iAnim);
...
BOOL CBasePlayerWeapon :: DefaultReload( int iClipSize, int iAnim, float fDelay, int body )
{
	...
	// buz: fDelay is now ignored, and replaced by animtime!
	WeaponDelay(iAnim);
...
/*void CBasePlayerWeapon::SpawnClip( void )
{	 
    if ( m_iClip >= 30 )
		return;

    if ( m_iClip <= 0 )
	    m_iWeaponMag = m_iEmptyMag;
    else
        m_iWeaponMag = m_iMag;
	...
	EjectBrass ( pev->origin + gpGlobals->v_up * -4 + gpGlobals->v_forward * 1 + gpGlobals->v_right * 10,vecClipVelocity, pev->angles.y, m_iWeaponMag, TE_BOUNCE_NULL);
}*/
...
void CBasePlayerWeapon::DrainClip(CBasePlayer* pPlayer, BOOL keep, int i9mm, int i357, int iBuck, int iBolt, int iARGren, int iRock, int iUranium, int iSatchel, int iSnark, int iTrip, int iGren,
								  int iAps, int iBarret, int iAk, int iAsval, int iRpk, int iGroza, int iTT33)
{
	...
	// buz: paranoia ammos
	else if (iPAI == pPlayer->GetAmmoIndex("aps"))		iAmt = iAps;
	else if (iPAI == pPlayer->GetAmmoIndex("barret"))	iAmt = iBarret;
	else if (iPAI == pPlayer->GetAmmoIndex("ak"))		iAmt = iAk;
	else if (iPAI == pPlayer->GetAmmoIndex("asval"))	iAmt = iAsval;
	else if (iPAI == pPlayer->GetAmmoIndex("rpk"))		iAmt = iRpk;
	else if (iPAI == pPlayer->GetAmmoIndex("groza"))	iAmt = iGroza;
    else if (iPAI == pPlayer->GetAmmoIndex("tt33"))	    iAmt = iTT33;
	else return;
...
{
	if( !stricmp( STRING( pev->model ), "models/w_weaponbox.mdl" ))
	{
...
// Wargon: SaveData для CRpg и CRpgRocket перемещены в rpg.cpp.

...
void CCrowbar::Smack( )
{
	UTIL_StudioDecalTrace( m_vecSrc, &m_trHit, "knife1" );
	UTIL_TraceCustomDecal( &m_trHit, "knife1" );
}

void CCrowbar::SmackNormal( )
{
	UTIL_StudioDecalTrace( m_vecSrc, &m_trHit, "knife2" );
	UTIL_TraceCustomDecal( &m_trHit, "knife2" );
} 
...
int CCrowbar::Swing( int iSecondary )
{
	...
	m_vecSrc = vecSrc;	// save off for studio decals
	...
	else
	{
		...
		if (fHitWorld)
		{
			...
			UTIL_KnifeImpact( &tr, vecSrc, vecSrc + (vecEnd-vecSrc)*2 );

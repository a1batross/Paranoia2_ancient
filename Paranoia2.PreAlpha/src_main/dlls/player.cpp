...
void LinkUserMessages( void )
{
	...
	gmsgGrass = REG_USER_MSG("GrassInfo", -1); //  buz
...
void CBasePlayer::RemoveItems( int iWeaponMask, int i9mm, int i357, int iBuck, int iBolt, int iARGren, int iRock, int iUranium, int iSatchel, int iSnark, int iTrip, int iGren, int iHornet,
							  int iAps, int iBarret, int iAk, int iAsval, int iRpk, int iGroza, int iPK, int iTT33)
{
	int i;
	...
	// buz: paranoia ammos
	RemoveAmmo("aps", iAps);
	RemoveAmmo("barret", iBarret);
	RemoveAmmo("ak", iAk);
	RemoveAmmo("asval", iAsval);
	RemoveAmmo("rpk", iRpk);
	RemoveAmmo("groza", iGroza);
	RemoveAmmo("painkillers", iPK);
    RemoveAmmo("tt33", iTT33);
	...
	for (i = 0; i < MAX_ITEM_TYPES; i++)
	{
		...
		while (pCurrentItem->m_pNext)
		{
			...
			{
				((CBasePlayerWeapon*)pCurrentItem)->DrainClip(this, FALSE, i9mm, i357, iBuck, iBolt, iARGren, iRock, iUranium, iSatchel, iSnark, iTrip, iGren, iAps, iBarret, iAk, iAsval, iRpk, iGroza, iTT33);
...
//				ALERT(at_console, "Removing %s. (id = %d)\n", pCurrentItem->m_pNext->pszName(), pCurrentItem->m_pNext->m_iId);
			...
			else
			{
				...
				((CBasePlayerWeapon*)pCurrentItem)->DrainClip(this, TRUE, i9mm, i357, iBuck, iBolt, iARGren, iRock, iUranium, iSatchel, iSnark, iTrip, iGren, iAps, iBarret, iAk, iAsval, iRpk, iGroza, iTT33);
...
void CBasePlayer::TabulateAmmo()
{
	...
	ammo_aps	= AmmoInventory( GetAmmoIndex( "aps" ) );
	ammo_barret	= AmmoInventory( GetAmmoIndex( "barret" ) );
	ammo_ak		= AmmoInventory( GetAmmoIndex( "ak" ) );
	ammo_asval	= AmmoInventory( GetAmmoIndex( "asval" ) );
	ammo_rpk	= AmmoInventory( GetAmmoIndex( "rpk" ) );
	ammo_groza	= AmmoInventory( GetAmmoIndex( "groza" ) );
    ammo_tt33	= AmmoInventory( GetAmmoIndex( "tt33" ) );
}
...
//	CheckDesiredList( ); //LRC
    ...
    if ( pev->button & IN_RUN && m_flStaminaValue > 0 )
	{
...
// buz test:
//	ALERT(at_console, "player max speed is %f\n", pev->maxspeed);
...
// dont let uninitialized value here hurt the player
	...
	if (startent->v.spawnflags & 2) // buz - the START WITH head shield flag
		pev->weapons |= (1<<WEAPON_HEADSHIELD);
...
void CSprayCan::Think( void )
{
	...
	if( pEntity && GET_MODEL_PTR( pEntity->edict() ))
		UTIL_StudioDecalTrace( pev->origin, &tr, "p2_sign2" );
	else UTIL_TraceCustomDecal( &tr, "p2_sign2" );
...
void CBloodSplat::Spray ( void )
{
	...
	if ( g_Language != LANGUAGE_GERMAN )
	{
		...
		if( pEntity && GET_MODEL_PTR( pEntity->edict() ))
			UTIL_BloodStudioDecalTrace( pev->origin, &tr, BLOOD_COLOR_RED );
...
void CBasePlayer::ImpulseCommands( )
{
	...
	case 45:
		if( m_pActiveItem )
			m_pActiveItem->WeaponSetMode( pev->impulse );
		break;
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 101:
		...
		GiveNamedItem( "item_gasmask" );
		GiveNamedItem( "item_headshield" );
		...
		GiveNamedItem( "weapon_aps" );
		GiveNamedItem( "weapon_9mmhandgun" );
		GiveNamedItem( "ammo_apsbox" );
		GiveNamedItem( "weapon_val" );
		GiveNamedItem( "ammo_valbox" );
		GiveNamedItem( "weapon_groza" );
		GiveNamedItem( "ammo_grozabox" );
		GiveNamedItem( "weapon_rpk" );
		GiveNamedItem( "ammo_rpkbox" );
		GiveNamedItem( "weapon_ak74" );
		GiveNamedItem( "weapon_aks" );
		GiveNamedItem( "ammo_aksbox" );
		GiveNamedItem( "weapon_tt33" );
		GiveNamedItem( "ammo_tt33box" );

		// Wargon: weapon_rpg и ammo_rpgclip перемещены из impulse 111.
...
*/
	...
	for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		if ( pWeapon )
		{
			...
			SET_MODEL( ENT(pWeaponBox->pev), STRING( pWeapon->m_iWorldModel ));
...
TYPEDESCRIPTION	CStripWeapons::m_SaveData[] = 
{
	...
	// buz: paranoia ammos
	DEFINE_FIELD( CStripWeapons, m_iAps, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iBarret, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iAk, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iAsval, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iRpk, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iGroza, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iPK, FIELD_INTEGER ),
	DEFINE_FIELD( CStripWeapons, m_iTT33, FIELD_INTEGER ),
...
// buz: paranoia ammos
	else if (FStrEq(pkvd->szKeyName, "aps"))
	{
		m_iAps = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "barret"))
	{
		m_iBarret = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "ak"))
	{
		m_iAk = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "asval"))
	{
		m_iAsval = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "groza"))
	{
		m_iGroza = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "rpk"))
	{
		m_iRpk = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
    else if (FStrEq(pkvd->szKeyName, "tt33"))
	{
		m_iTT33 = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "painkillers"))
	{
		m_iPK = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "gasmask"))
	{
...
void CStripWeapons :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
	{
		// buz: dont touch my gasmask and shield!
		pev->spawnflags |= (1<<WEAPON_GASMASK | 1<<WEAPON_HEADSHIELD);
		pPlayer->RemoveItems( pev->spawnflags, m_i9mm, m_i357, m_iBuck, m_iBolt,
				...
				m_iAps, m_iBarret, m_iAk, m_iAsval, m_iRpk, m_iGroza, m_iPK, m_iTT33);

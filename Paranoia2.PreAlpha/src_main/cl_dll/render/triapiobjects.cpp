...
void CalcViewMatrix( int reset = 0 )
{
	...
	AngleMatrix( RI.refdef.viewangles, g_viewMatrix );
...
void CObjectsManager::Draw( void )
{
	...
	while(currentObject)
	{
		if( currentObject->Draw( RI.refdef.time, RI.refdef.frametime ))
		{
...
void MakeUpRight( const float *normal, float *up, float *right )
{
	...
	{
		right = CrossProduct( normal, temp );
...
CGunSmoke::CGunSmoke( vec3_t origin, vec3_t angles, float life, float alpha, float minspeed, float maxspeed, float minsize, float maxsize, float maxofs, float r, float g, float b)
{
	...
	for(int i = 0; i < 5; i++)
	{
		...
		m_puffsArray[i].size = gEngfuncs.pfnRandomFloat(minsize, maxsize);
		VectorClear(m_puffsArray[i].velocity);
		VectorMA(m_puffsArray[i].velocity, gEngfuncs.pfnRandomFloat(minspeed, maxspeed), angles, m_puffsArray[i].velocity);
		VectorMA(m_puffsArray[i].velocity, gEngfuncs.pfnRandomFloat(-maxofs, maxofs), forward, m_puffsArray[i].velocity);
		VectorMA(m_puffsArray[i].velocity, gEngfuncs.pfnRandomFloat(-maxofs, maxofs), right, m_puffsArray[i].velocity);
		VectorMA(m_puffsArray[i].velocity, gEngfuncs.pfnRandomFloat(-maxofs, maxofs), up, m_puffsArray[i].velocity);
	}
...
int CGunSmoke::Draw(float time, float frametime)
{
	float frac = (time - birthTime) / m_fLife;
	if (frac > 1)
		return 1; // delete object

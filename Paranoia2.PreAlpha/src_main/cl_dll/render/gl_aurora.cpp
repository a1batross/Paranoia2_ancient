...
CParticleSystem :: CParticleSystem( cl_entity_t *ent, const char *szFilename, int attachment, float lifetime )
{
	...
	else
	{
		...
		while( pfile )
		{
			...
			{
				pfile = COM_ParseFile( pfile, szToken );
				m_fLifeTime = RI.refdef.time + Q_atof( szToken );
			}
...
void CParticleSystem :: CalculateDistance( void )
{
	...
	vec3_t offset = RI.vieworg - m_pActiveParticle->origin; // just pick one
...
AURSTATE CParticleSystem :: UpdateSystem( float frametime )
{
	...
	if( m_pEntity != NULL )
	{
		...
		if( m_iEntAttachment && m_pEntity->model->type == mod_studio )
			entityMatrix.FromVector( m_pEntity->ph[m_iEntAttachment-1].angles );
		...
		if( enable )
		{
			...
			if( pType )
			{
				...
				if( m_pMainParticle )
				{
					...
					if( m_iEntAttachment && m_pEntity->model->type == mod_studio )
						m_pMainParticle->origin = m_pEntity->ph[m_iEntAttachment-1].origin;
...
bool CParticleSystem :: UpdateParticle( CParticle *part, float frametime )
{
	...
	if( part->m_pEntity )
	{
		if( enable )
		{
			if( m_iEntAttachment && m_pEntity->model->type == mod_studio )
			{
				float flSpeed = (m_pEntity->ph[m_iEntAttachment-1].origin - part->origin).Length() * frametime;
				part->velocity = entityMatrix.GetForward() * flSpeed;
				part->origin = m_pEntity->ph[m_iEntAttachment-1].origin;
			}
			else
			{
...
void CParticleSystem :: DrawParticle( CParticle *part, vec3_t &right, vec3_t &up )
{
	...
	for( CParticle *pDraw = part; pDraw; pDraw = pDraw->m_pOverlay )
	{
		...
		if( m_iLightingModel >= 1 )
		{
			...
			R_LightForPoint( part->origin, &lightinfo, false );
			
			// FIXME: this code is totally wrong.
			// We need a fake lightmap here like in sprite implementation
			lightingColor.x = pDraw->m_fRed * lightinfo.ambient.x;
			lightingColor.y = pDraw->m_fGreen * lightinfo.ambient.y;
			lightingColor.z = pDraw->m_fBlue * lightinfo.ambient.z;
		...
		if( m_iLightingModel >= 2 && m_fHasProjectionLighting )
		{
			for( int i = 0; i < MAX_DLIGHTS; i++ )
			{
				DynamicLight *pl = &cl_dlights[i];

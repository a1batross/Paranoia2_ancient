...
void CSpriteModelRenderer::SpriteDrawModel( void )
{
	...
	if( m_pCurrentEntity->curstate.renderfx == SKYBOX_ENTITY )
	{
		...
		if( tr.sky_speed )
		{
			...
			tr.modelorg = skypos - origin;
		}
		else
		{
			tr.modelorg = tr.sky_origin - origin;
                    }

		origin += trans; // move to the sky position
	...
	{
		lightinfo_t light;
		R_LightForPoint( origin, &light, false ); // get static lighting
		color2 = light.ambient;
...
void CSpriteModelRenderer::SpriteDrawModelShadowPass( void )
{
	...
	if( tr.cached_refdef != NULL )
	{
		// g-cont. this is a very stupid case but...
		matrix3x3	vectors( tr.cached_refdef->viewangles - RI.refdef.viewangles );

...
//	side = V_CalcRoll ( viewentity->angles, pparams->simvel, 5, 200 ); // buz test

//	pparams->viewangles[PITCH] += side;
...
void V_Init (void)
{
	...
	cv_bumpdebug	= CVAR_REGISTER( "bump_debug", "0", 0 );
	cv_specular	= CVAR_REGISTER( "gl_specular", "1", FCVAR_ARCHIVE );
	cv_dynamiclight	= CVAR_REGISTER( "gl_dynlight", "1", FCVAR_ARCHIVE );
	cv_specular_nocombiners = CVAR_REGISTER( "gl_nocombs", "0", 0 );
	cv_specular_noshaders = CVAR_REGISTER( "gl_noshaders", "0", 0 );
	cv_highspecular	= CVAR_REGISTER( "gl_highspecular", "1", FCVAR_ARCHIVE );
	cv_parallax	= CVAR_REGISTER( "gl_parallax", "1", FCVAR_ARCHIVE );
	cv_gamma		= CVAR_REGISTER( "gl_gamma","1", FCVAR_ARCHIVE );
	cv_brightness	= CVAR_REGISTER( "gl_brightness","0", FCVAR_ARCHIVE );
	cv_contrast	= CVAR_REGISTER( "gl_contrast","1", FCVAR_ARCHIVE );
	cv_speculartwopass	= CVAR_REGISTER( "gl_twopassspecular","0", 0 );
	cv_dyntwopass	= CVAR_REGISTER( "gl_twopassdyn","0", FCVAR_ARCHIVE );
	cv_blurtest	= CVAR_REGISTER( "gl_blurtest","0", 0 );
	cv_water		= CVAR_REGISTER( "gl_water", "1", FCVAR_ARCHIVE );
	cv_decalsdebug	= CVAR_REGISTER( "gl_decalstatus", "0", 0 );
	r_newrenderer	= CVAR_REGISTER( "gl_newrenderer", "1", FCVAR_ARCHIVE );
	r_separate_textures	= CVAR_REGISTER( "gl_separate_textures", "0", FCVAR_ARCHIVE );
...
void SetupFlashlight( struct ref_params_s *pparams )
{
	...
	Vector origin = pparams->vieworg + Vector( 0.0f, 0.0f, -13.0f ) + (pparams->right * 5.0f);
	...
	DynamicLight *flashlight = MY_AllocDlight( FLASHLIGHT_KEY );

	R_SetupLightProjection( flashlight, origin, pparams->viewangles, 700.0f, 35.0f + add, tr.flashlightTexture );

	flashlight->color = Vector( 1.4f, 1.4f, 1.4f ); // make model dymanic lighting happy
	flashlight->die = pparams->time + 0.01f;

	if( texName && !Q_strnicmp( texName, "reflect", 7 ) && r_allow_mirrors->value )
	{
		DynamicLight *flashlight2 = MY_AllocDlight( FLASHLIGHT_KEY + 1 );
		float d = 2.0f * DotProduct( pparams->forward, ptr.plane.normal );
		Vector angles, dir = (pparams->forward - ptr.plane.normal) * d;
		VectorAngles( dir, angles );

		R_SetupLightProjection( flashlight2, ptr.endpos, angles, 700.0f, 35.0f + add, tr.flashlightTexture );

...
public:
			...
			if( *c == '.' )
			{
				if( cOneDot != NULL )
				{
					...
					*cOneDot = 0; // null terminate the first number
					m_fMin = Q_atof( szToken ); // parse the first number
					*cOneDot = '.'; // change it back, just in case
					m_fMax = Q_atof( c + 1 ); // parse the second number
					return;
				}
				else cOneDot = c;
			}
	...
	float GetInstance() { return RANDOM_FLOAT( m_fMin, m_fMax ); }
	float GetOffset( float fBasis ) { return GetInstance() - fBasis; }

...
#if 0	// don't cull local player because we draws legs instead
	if( RP_LOCALCLIENT( e ) && !RI.thirdPerson && UTIL_IsLocal( RI.refdef.viewentity ))
	{
		// player can view himself from the portal camera
		if(!( RI.params & ( RP_MIRRORVIEW|RP_SHADOWVIEW )))
			return true;
	}
#endif
	if( R_CullBox( origin + mins, origin + maxs, RI.clipFlags ))
		return true;

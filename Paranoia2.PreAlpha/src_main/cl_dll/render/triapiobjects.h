...
class CGunSmoke : public CObject
{
public:
	// construction
	CGunSmoke( vec3_t origin, vec3_t normal, float life, float alpha, float minspeed, float maxspeed, float minsize, float maxsize, float maxofs, float r=1.0, float g=1.0, float b=1.0 );
	~CGunSmoke() {gEngfuncs.Con_Printf("deleting puff\n");};
	int Draw( float time, float frametime );

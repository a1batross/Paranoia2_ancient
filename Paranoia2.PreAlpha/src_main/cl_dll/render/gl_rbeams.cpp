...
void CL_DrawBeamFollow( int spriteIndex, BEAM *pbeam, int frame, int rendermode, float frametime, float *color )
{
	...
	if( pbeam->flags & FBEAM_STARTENTITY )
	{
#if 0	// this code provoke hanging...
		cl_entity_t *ent = GET_BEAM_ENTITY( pbeam->startEntity );

		// beam was teleported
		if( ent && (ent->curstate.origin - ent->prevstate.origin).Length() == 0 )
		{
			if( CL_NumBeamsForEntity( ent ) <= 1 )
			{
				float life = (pbeam->die - GET_CLIENT_TIME());
				gEngfuncs.pEfxAPI->R_BeamKill( pbeam->startEntity );
				gEngfuncs.pEfxAPI->R_BeamFollow( pbeam->startEntity, pbeam->modelIndex, life, pbeam->width,
				pbeam->r, pbeam->g, pbeam->b, pbeam->brightness );
				return;
			}
		}
#endif

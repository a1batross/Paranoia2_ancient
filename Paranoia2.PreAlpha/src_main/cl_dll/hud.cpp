...
int MsgCustomDlight(const char *pszName, int iSize, void *pbuf)
{
	...
	R_SetupLightProjection( dl, pos, g_vecZero, radius, 0.0f );
...
void MakeLight( void )
{
	...
	R_SetupLightProjection( dl, RI.vieworg, g_vecZero, radius, 0.0f );

	if( CMD_ARGC() >= 6 )
		dl->die = GET_CLIENT_TIME() + Q_atof(CMD_ARGV( 5 ));
	else dl->die = GET_CLIENT_TIME() + 2.0f;
}
...
void CHud :: Init( void )
{
	...
	CVAR_REGISTER( "gl_blur", "0", FCVAR_ARCHIVE );
	CVAR_REGISTER( "gl_blur_strength", "1", FCVAR_ARCHIVE );
...
//	default_fov = CVAR_REGISTER( "default_fov", "90", 0 );// buz: turn off
	...
	CVAR_REGISTER( "fmod_volume", "1.0", FCVAR_ARCHIVE );

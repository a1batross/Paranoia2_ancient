...
int CHudLensflare :: DrawFlare( const Vector &forward, const Vector &lightdir, const Vector &lightorg )
{
	...
	gEngfuncs.pTriAPI->WorldToScreen( (float *)&lightorg, screen );
...
{  
	...
	for( int i = 0; i < MAX_DLIGHTS && m_pCvarDraw->value > 1.0f; i++ )
	{
		DynamicLight *pl = &cl_dlights[i];

		if( pl->die < flTime || !pl->radius || !pl->spotlightTexture )
			continue;

		sundir = -pl->frustum[5].normal;
		suntarget = pl->origin;

		gEngfuncs.pEventAPI->EV_SetTraceHull( 2 );
		gEngfuncs.pEventAPI->EV_PlayerTrace( g_pViewParams->vieworg, suntarget, PM_STUDIO_IGNORE, -1, &tr );

		if( DotProduct( forward, sundir ) >= 0.68f && !R_CullSphereExt( pl->frustum, RI.vieworg, 72, pl->clipflags ) && tr.fraction == 1.0f )
		{		
			DrawFlare( forward, sundir, suntarget );
		}
	}

	if( g_pViewParams != NULL && g_pViewParams->movevars != NULL )
	{
		Vector skyVec = Vector( g_pViewParams->movevars->skyvec_x, g_pViewParams->movevars->skyvec_y, g_pViewParams->movevars->skyvec_z );

		if( skyVec == g_vecZero )
			return 1;	// no light_environment on a map

		sundir = -skyVec;
		suntarget = g_pViewParams->vieworg + sundir * 16384;	

		tr = *(gEngfuncs.PM_TraceLine( g_pViewParams->vieworg, suntarget, 0, 2, -1 ));

		if( DotProduct( forward, sundir ) >= 0.68f && R_SkyIsVisible() && gEngfuncs.PM_PointContents( tr.endpos, null ) == CONTENTS_SKY )
		{		
			DrawFlare( forward, sundir, suntarget );
		}
	}
